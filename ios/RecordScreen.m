...
//  CheckOn
//
//  Created by memorres on 03/01/24.
//
...
- (void)sendEvent {
  if (hasListeners) {
    [self sendEventWithName:@"SomeEvent" body:@{@"body": self.someBody}];
  }
}
...
RCT_REMAP_METHOD(startRecording, resolve:(RCTPromiseResolveBlock)resolve rejecte:(RCTPromiseRejectBlock)reject)
{
    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
      if (granted) {
            if (@available(iOS 11.0, *)) {
              [self sendEventWithName:@"onSessionConnect" body: @{@"getPermission": @(granted)}];
                [self.screenRecorder startCaptureWithHandler:^(CMSampleBufferRef sampleBuffer, RPSampleBufferType bufferType, NSError* error) {
                [self sendEventWithName:@"onSessionConnect" body: @{@"permissionGranted": @(granted)}];
            ...
            [[RPScreenRecorder sharedRecorder] stopCaptureWithHandler:^(NSError * _Nullable error) {
                if (!error) {
                    ...
                    [self.writer finishWritingWithCompletionHandler:^{
                      ...
                      [self sendEventWithName:@"onSessionConnect" body: @{@"recordingComplete": @(TRUE)}];
