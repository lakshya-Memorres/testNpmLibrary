...
import com.recordscreen.ShareFile;
import com.recordscreen.ShareFiles;
...
public abstract class ShareIntent {
    ...
    public void open(ReadableMap options) throws ActivityNotFoundException {
        ...
        if (ShareIntent.hasValidKey("urls", options)) {
          Log.d(TAG, "options log ===>>" + options);
            ShareFiles fileShare = getFileShares(options);
          Log.d(TAG, "fileShare ===>>" + fileShare);
            if (fileShare.isFile()) {
              Log.d(TAG, "if block ===>>" );
